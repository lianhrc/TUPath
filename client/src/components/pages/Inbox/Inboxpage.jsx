...
function Inboxpage() {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        const response = await axiosInstance.get(`/api/messaging/conversations/${activeConversationId}/messages`);
        if (response.data.success) {
  ...
  const handleSearchChange = async (e) => {
    ...
    if (query.length > 0) {
      ...
      try {
        ...
        const response = await axiosInstance.get(`/api/messaging/search?query=${query}`);
        if (response.data.success) {
  ...
  const handleCreateConversation = async () => {
    ...
    try {
      const response = await axiosInstance.post('/api/messaging/conversations', {
      ...
      if (response.data.success) {
        ...
        if (!response.data.message || response.data.message !== "Conversation already exists") {
          // Ensure the conversation has the proper name
          const updatedNewConvo = {
            ...newConvo,
            participants: newConvo.participants.map(p => {
              if (p.userId._id === newConversationName.id || p.userId === newConversationName.id) {
                // This is the participant we just selected, make sure they have the right name
                return {
                  ...p,
                  userId: {
                    ...p.userId,
                    username: typeof newConversationName === 'object' ? newConversationName.name : newConversationName
                  }
                };
              }
              return p;
          ...
          const existingConvo = conversations.find(c => 
            c.participants.some(p => 
              (p.userId._id === newConversationName.id || p.userId === newConversationName.id)
            )
          );
          if (existingConvo) {
        ...
        const messagesResponse = await axiosInstance.get(`/api/messaging/conversations/${newConvo._id}/messages`);
  ...
  const activeConversation = conversations.find(convo => convo._id === activeConversationId);
            ...
            {showNewConversation && (
              <div className="new-conversation-form">
                <div className="search-container" ref={dropdownRef}>
                  <input
                    type="text"
                    value={typeof newConversationName === 'object' ? newConversationName.name : newConversationName}
                              ...
                              <div className="search-result-item">
                                ...
                                <div className="search-user-details">
                                  ...
                                  {user.role === 'student' && user.bestTag && (
                                    <span className="user-tag">{user.bestTag}</span>
                                  )}
                                  {user.role === 'employer' && user.profileDetails.companyName && (
                                    <span className="company-name">{user.profileDetails.companyName}</span>
                                  )}
                                </div>
                              </div>
                            </li>
          ...
          <div className="conversation-list">
            {isLoading ? (
              ...
              conversations.map((convo) => {
                ...
                const conversationName = otherParticipant?.username || 'Unknown';
                ...
                return (
                  ...
                  >
                    <div className="sender-info">
                      ...
                      <p className="message-preview">
                        {convo.lastMessage ? (
                          ...
                          (() => {
                            const lastMessageSender = convo.lastMessage.sender._id || convo.lastMessage.sender;
                            const wasCurrentUser = lastMessageSender === currentUserId || 
                                                  lastMessageSender.toString() === currentUserId.toString();
                            return `${wasCurrentUser ? 'You: ' : `${conversationName}: `}${convo.lastMessage.content}`;
                          })()
                        ) : 'No messages'}
                      </p>
                    </div>
                    <div className="message-meta">
                      <span className="time">
                        {convo.lastMessage ? formatMessageDate(convo.lastMessage.createdAt) : ''}
                      </span>
              ...
              <div className="message-header">
                <div className="conversation-profile">
                  ...
                  <div className="profile-info">
                    <h2>{getOtherParticipant(activeConversation)?.username || 'Unknown'}</h2>
              ...
              <div className="messages-container">
                {messages.length === 0 ? (
                  <div className="no-messages">
                    <p>No messages yet</p>
                    <p className="start-conversation">Start the conversation with {getOtherParticipant(activeConversation)?.username}</p>
                  </div>
                ) : (
                  <>
                    ...
                    {messages.map((msg) => {
                      ...
                      const isCurrentUser = senderId === currentUserId || senderId.toString() === currentUserId.toString();
                      const senderName = isCurrentUser ? "You" : (msg.sender.username || "sUnknown");
                      
                      return (
                        <div 
                          key={msg._id} 
                          className={`message-wrapper ${isCurrentUser ? 'sent-wrapper' : 'received-wrapper'}`}
                        >
                          <div className={`message ${isCurrentUser ? 'message-sent' : 'message-received'}`}>
                            {!isCurrentUser && <span className="message-sender">{senderName}</span>}
