...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const response = await axiosApi.get('/check-auth');
  ...
  const handleLogout = async () => {
    try {
      ...
      const response = await axiosApi.post('/api/admin/logout');
      if (response.data.success) {
...
const UsersSection = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      setLoading(true);
      try {
        const response = await axiosApi.get(`/api/users?type=${selectedType}`);
        if (response.data.success) {
    ...
    const fetchUserStats = async () => {
      try {
        const response = await axiosApi.get('/api/user-stats');
        if (response.data.success) {
          setUserStats({
  ...
  const handleDelete = async (userId) => {
    ...
    try {
      const response = await axiosApi.delete(`/api/manage-users/${userId}?type=${selectedType}`);
      if (response.data.success) {
...
const TagChart = () => {
  ...
  useEffect(() => {
    const fetchTagData = async () => {
      try {
        const response = await axiosApi.get('/api/student-tags');
        if (response.data.success) {
  ...
  const handleBarClick = async (event, elements) => {
    if (elements.length > 0) {
      ...
      try {
        const response = await axiosApi.get('/api/students-by-tag', { params: { tag } });
        if (response.data.success) {
